...
export function versionCheck() {
  ...
  const node = selection?.nodes?.[0] as EditorNodeInstance<HassNodeProperties>;
  ...
  const str = type
    ...
    .reduce(
      (acc: string[], cur, index) =>
        acc.concat(`${index ? cur[0].toUpperCase() : cur[0]}${cur.slice(1)}`),
      []
    )
    .join('');
...
function isHomeAssistantConfigNode(
  ...
  return (
    node?.type &&
    [NodeType.Server, NodeType.EntityConfig, NodeType.DeviceConfig].includes(
      node.type
    )
  );
}
