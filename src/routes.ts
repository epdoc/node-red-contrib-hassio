...
async function getDeviceTriggerCapabilities(
  ...
  const capabilities =
    await req?.homeAssistant?.websocket.getDeviceTriggerCapabilities(trigger);
  res.json(capabilities ?? []);
}
...
function getProperties(req: CustomRequest, res: Response): void {
  ...
  if (Array.isArray(entity)) {
    flat = Object.keys(flatten(entity)).filter(
      (e) => req?.query?.term && e.indexOf(req.query.term.toString()) !== -1
    );
  } else {
  ...
  const uniqProperties = Array.from(new Set(([] as string[]).concat(...flat)));
