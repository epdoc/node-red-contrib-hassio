...
export default class Websocket {
  ...
  async #haEvents() {
    ...
    subscribeConfig(this.client, (config) => this.onClientConfigUpdate(config));
  ...
  async subscribeEvents(events: { [nodeId: string]: string }): Promise<void> {
    ...
    for (const type of add) {
      this.#unsubCallback[type] = await this.client.subscribeEvents<HassEvent>(
  ...
  getStates(entityId?: unknown): unknown {
    if (typeof entityId === 'string') {
      return this.states[entityId] ? cloneDeep(this.states[entityId]) : null;
    }
