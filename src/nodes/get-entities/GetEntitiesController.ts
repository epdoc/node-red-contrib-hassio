  ...
  protected async onInput({
    ...
    for (const entity of entityArray) {
      ...
      for (const rule of rules) {
        ...
        if ((rule.logic !== 'jsonata' && value === undefined) || !result) {
          entityMatched = false;
          break;
        }
      }
    ...
    switch (parsedMessage.outputType.value) {
      ...
      case OutputType.Random: {
        ...
        const max =
          entities.length <= maxReturned ? entities.length : maxReturned;
      ...
      default:
        if (entities.length === 0 && !parsedMessage.outputEmptyResults.value) {
          noPayload = true;
        }
